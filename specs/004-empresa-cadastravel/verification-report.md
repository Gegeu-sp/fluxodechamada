...
- Antes do deploy real: republicar `firestore.rules` no Console (novo bloco `empresas` + `turmas` agora exige `empresa is string`) e, como já recomendado na feature 003, reabrir/salvar de novo as turmas de teste existentes escolhendo também uma empresa.
